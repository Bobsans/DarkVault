...
public static class VaultApplication {
    public static WebApplication Build(string[] args, VaultStore store, KeyRing ring, string keyDirectory) {
        ...
        builder.Services.AddSingleton<AdminSessions>(); builder.Services.AddRazorPages();
        ...
        app.UseRateLimiter(); app.UseStaticFiles(); app.MapRazorPages();
        app.MapGet("/health/ready", () => { store.Ready(); return Results.Json(new { status = "ready" }); });
        app.MapGet("/api/v1/crypto/key", () => ring.Public()).RequireRateLimiting("api");
        app.MapGet("/admin/api/v1/session", (HttpContext c, IAntiforgery csrf, AdminSessions sessions) => Results.Json(new { authenticated = sessions.Get(c, store) is not null, csrfToken = csrf.GetAndStoreTokens(c).RequestToken }));
        app.MapPost("/admin/login", async (HttpContext c, IAntiforgery csrf, AdminSessions sessions) => {
            ...
            var credentials = Wire.Parse<LoginRequest>(await Wire.ReadBodyAsync(c.Request.Body, c.RequestAborted));
            ...
            sessions.Create(c, admin); await c.Response.WriteAsJsonAsync(new { authenticated = true });
        }).RequireRateLimiting("login");
        app.MapPost("/admin/logout", async (HttpContext c, IAntiforgery csrf, AdminSessions sessions) => {
            if (!await csrf.IsRequestValidAsync(c)) { await Error(c, 400, "csrf_failed"); return; }
            sessions.Remove(c); await c.Response.WriteAsJsonAsync(new { authenticated = false });
        });
        app.MapPost("/admin/password", async (HttpContext c, IAntiforgery csrf, AdminSessions sessions) => {
            ...
            var input = Wire.Parse<PasswordRequest>(await Wire.ReadBodyAsync(c.Request.Body, c.RequestAborted));
            if (store.Login(input.CurrentPassword) is null) throw new VaultFault(401, "unauthorized");
            store.SetPassword(input.NewPassword, true); sessions.Clear(); sessions.Remove(c); await c.Response.WriteAsJsonAsync(new { changed = true });
        }).RequireRateLimiting("login");
    ...
    private static async Task Execute(HttpContext c, bool admin, VaultStore store, KeyRing ring, AdminSessions sessions, IAntiforgery csrf, SemaphoreSlim admission) {
        ...
        try {
            ...
            var response = new VaultResponse(1, request.RequestId, ring.ServerId, admin ? "admin" : "data", request.Operation, status, data is null ? null : JsonSerializer.SerializeToElement(data, Wire.Json), error);
            var encrypted = Wire.Encrypt(Wire.Serialize(response), request.ReplyKey, request.RequestId, "darkvault-response+jwe");
    ...
    private static Task Error(HttpContext c, int status, string code) { c.Response.StatusCode = status; return c.Response.WriteAsJsonAsync(new { error = new VaultError(code, "Request rejected.") }); }
