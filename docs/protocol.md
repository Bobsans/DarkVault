...
Отсутствующий или `null` `expiresAt` при выпуске токена означает 30 дней;
максимум — один календарный год: та же дата и время следующего года в UTC,
для 29 февраля — 28 февраля. Сохранённый явный срок существующих токенов не продлевается. Ранее выданные токены также ограничены одним календарным годом от CreatedAt.

`POST /admin/login` принимает username/password и необязательный recoveryCode.
Успех возвращает `{authenticated:false,mode:"register"|"verify",options:{...}}`:
это WebAuthn options, административной сессии ещё нет. Браузер вызывает
`navigator.credentials.create/get` и отправляет `{credential: credential.toJSON()}`
в `POST /admin/mfa/verify`. Только успешная проверка создаёт сессию и возвращает
`{authenticated:true,recoveryCodes:[...]}`. Коды выдаются один раз при первичной
регистрации или восстановлении. Challenge привязан к Secure/HttpOnly cookie,
origin, текущему stamp администратора и сроку две минуты; повтор запрещён.
Все MFA POST требуют CSRF и корректный Origin, user verification обязательна.

`POST /admin/mfa/options` требует сессию и выдаёт challenge для повторного
подтверждения. `POST /admin/mfa/register` требует сессию и свежую MFA, затем
выдаёт challenge для резервного passkey. Максимум восемь passkeys.
Создание токена, удаление бакета, смена пароля и добавление passkey требуют
MFA не старше пяти минут; иначе `403 reauth_required`. Ответы об ограничениях
или временной блокировке — `429 rate_limited` с `Retry-After` в секундах.

Настройки и восстановление: [security.md](security.md).

`audit.list` возвращает события `kind: operation` (выполнение команды) и `kind: http`
(завершение аутентифицированного запроса, кроме `429`). Анонимный трафик и `429`
не записываются в SQLite; отказы проверки пароля, MFA и токена поступают в
структурированный журнал `DarkVault.Security`, все запросы учитываются метрикой
`darkvault.http.requests`. Это исключает запись в БД для каждого запроса атаки.
Поля `principal`, `principalType`, `principalName` описывают проверенную учётную запись.
`time` и `startedAt` — UTC, `sourceIp` — адрес после обработки доверенного proxy,
`peerIp` — адрес TCP-соединения, `method`/`path` — HTTP-метод и путь без query string,
`statusCode` — HTTP-статус, `durationMs` — длительность. `traceId` связывает события
одного HTTP-запроса; `requestId` сохраняет UUID проверенной JWE-команды, а до её
проверки равен серверному traceId. У локальных операций сетевые поля равны null.

`details` содержит только метаданные: запрошенные `bucket`, `key`, `tokenId`,
`expectedRevision`, `recursive`, массив `resources` (тип, UUID, имя, UUID бакета,
ревизия), `returnedCount` и права созданного/просмотренного токена `token` без
самого credential. При отказе массив resources пуст: это не успешная выдача данных.
Для массовой операции resources разбивается на записи по 200 объектов; requestId,
traceId и общее returnedCount одинаковы. `returnedCount` для удаления означает
число удалённых объектов. Список аудита фиксирует число возвращённых записей,
не копируя их содержимое в собственное событие.

Страница может содержать меньше `limit` записей из-за лимита размера ответа;
всегда следуйте `nextCursor`. Исторические записи могут не иметь новых полей.
Необязательные параметры `audit.list`: `order` (`asc` по умолчанию или `desc`),
`search` (до 256 символов, подстрока операции, учётной записи, IP, пути, идентификаторов
или метаданных объектов), `kind` (`operation`/`http`) и `result` (`success`/`failure`).
Пустые строки означают отсутствие фильтра; `%` и `_` в поиске считаются обычными
символами. Фильтры применяются до пагинации; следующий cursor нужно использовать
с теми же фильтрами и порядком. Для новых событий в режиме desc начните без cursor.
Расширенные события отображаются существующим admin UI без изменения публичных SDK.
Значения секретов, пароли, credentials, CSRF, тела и произвольные заголовки в аудит
не записываются. Политика хранения и эксплуатационные ограничения — в
[спецификации](specification.md#9-аудит-и-эксплуатация).

## 6. Ошибки и лимиты

Секреты поддерживают `type: string|number|boolean|null` (по умолчанию string).
`value` на wire остаётся строкой с каноническим представлением выбранного типа.
Тип возвращается в метаданных секрета; bucket.read дополняется разреженным
словарём `types` (отсутствующий ключ — string). Недопустимый тип или значение
возвращает HTTP 400 `invalid_secret_type`. Правила чисел, совместимости хранения,
вложенности и API клиентов описаны в [configuration.md](configuration.md).
