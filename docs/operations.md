...
docker run --rm -it `
  ...
  docker.io/bobsans/darkvault:2.1.0 bootstrap
```
...
Caddy получает и обновляет сертификат домена. При обычном запуске без контейнера
...
Приоритет подключения: явно заданные флаги → непустая `DARKVAULT_URL` → остальные непустые переменные окружения →
...
`rotate-data` перешифровывает записи транзакциями, а после успешного прохода удаляет неиспользуемые старые data keys;
при сбое до завершения старые ключи сохраняются для восстановления, поэтому повтор безопасен.
...
CLI автоматически читает секретную переменную `DARKVAULT_URL`:

```text
https://<token>@vault.site.com/bucket-name
https://<token>@localhost:8443/bucket-name
```

Обязательны HTTPS, токен `dv1_` и имя бакета. Допустимы порт и IPv6 в квадратных
скобках. Имя бакета — 1–63 символа `a-z`, `0-9`, `_`, `-`, первый символ — буква
или цифра. Используются буквальные значения без percent encoding.
Пароль, дополнительный путь, завершающий `/`, query и fragment не допускаются.
Строку целиком храните как секрет CI; не выводите её в логи.

Приоритет сервера и токена: явные флаги → `DARKVAULT_URL` →
раздельные переменные окружения → сохранённый конфиг. Невалидная непустая
`DARKVAULT_URL` приводит к ошибке. Таймаут и размер страницы настраиваются как раньше.
CLI не сохраняет эту строку в конфиг.

Когда переменная задана, бакет можно пропускать:

```sh
darkvault bucket read --format nested-json
darkvault secret get ApiKey
darkvault exec -- dotnet MyApp.dll
```

Явный бакет в аргументах или `exec --bucket` имеет приоритет.
Для `bucket add` имя создаваемого бакета всегда обязательно.
`exec` исключает `DARKVAULT_URL` из наследуемого окружения дочернего процесса,
как и `DARKVAULT_TOKEN` / `DARKVAULT_TOKEN_FILE`.
...
### Загрузка настроек одним вызовом

```csharp
builder.Configuration.AddFromDarkVault(url);
// Asynchronous startup:
await builder.Configuration.AddFromDarkVaultAsync(url);
```

Синхронный вариант блокирует вызывающий поток до окончания загрузки; используйте
его при старте приложения. Оба варианта добавляют данные только после успешной
проверки ответа и ключей. Последующие configuration providers могут переопределить
эти значения. Отмена и свой `HttpClient` доступны через необязательные аргументы.
Автоматического обновления нет.

В остальных SDK доступны короткие методы:

- C#: `await DarkVaultClient.LoadConfigurationAsync(url, SettingsJsonContext.Default.AppSettings)`.
  Нужен source-generated `JsonTypeInfo<T>` для совместимости с Native AOT.
- Python: `load_configuration(url)` из `darkvault` возвращает вложенный словарь;
  соединение закрывается и при успехе, и при ошибке.
- TypeScript: `await loadConfiguration(url)` из `@darkvault/client`.
- Go: `darkvault.LoadConfiguration(ctx, url, &settings)` заполняет структуру или map;
  ошибку нужно обработать обычным способом.

Типы чисел, boolean и null сохраняются. Во всех случаях это однократная загрузка;
для повторных операций и дополнительных настроек используйте существующий клиент.

## Переименование бакета

В веб-интерфейсе откройте бакет → **Bucket settings**, измените **Name** и нажмите **Save**.
UUID, секреты и доступ токенов сохраняются. После переименования обновите имя
бакета в конфигурациях приложений и connection URL: старое имя больше не работает.
